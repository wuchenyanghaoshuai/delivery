...
class PrepareEnvHandler implements Serializable {
	...
	def handle() {
		...
		if (env.gitlabSourceBranch != null) {
            env.VERSION_PREFIX = env.gitlabSourceBranch
            if (env.VERSION_PREFIX.contains('release') || env.VERSION_PREFIX.contains('bugfix') || env.VERSION_PREFIX.contains('origin/fix')){
                env.VERSION_PREFIX = env.VERSION_PREFIX.replaceAll(".*release/",'').replaceAll(".*fix/",'')
            } else {
                env.VERSION_PREFIX = env.VERSION_PREFIX.replace('origin/','')
            }
        }
		// USER ID
